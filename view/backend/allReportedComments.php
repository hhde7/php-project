                    ...
                    if (isset($reportedComments[$i])) { // EVITE UNE ERREUR SI LES 20 ÉLÉMENTS NE SONT PAS TROUVÉS
						...
						<tr>
							...
							<td class="table-options">
								<a href="index.php?action=reportedComments&amp;see=<?= $reportedComments[$i]->getCommentId() ?>&amp;page=<?= htmlspecialchars($_GET['page']) ?>" title="Voir"><i class="fas fa-plus-circle"></i></a>
								<a href="index.php?action=moderate&amp;allow=<?= $reportedComments[$i]->getCommentId() ?>&amp;from=reportedComments&amp;page=<?= htmlspecialchars($_GET['page']) ?>" title="Accepter"><i class="fas fa-check-circle"></i></a>
								<a href="index.php?action=moderate&amp;delete=<?= $reportedComments[$i]->getCommentId() ?>&amp;from=reportedComments&amp;page=<?= htmlspecialchars($_GET['page']) ?>" title="Supprimer"><i class="fas fa-minus-circle"></i></a>
							</td>
						</tr>
					<?php
                    }
