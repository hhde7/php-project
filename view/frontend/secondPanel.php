...
<h2 class="second-panel-title">BILLET SIMPLE POUR L'ALASKA - ÉPISODE <?= mb_strimwidth($episode->getTitle(), 0, 2) ?> <i class="fab fa-envira"></i></h2>
...
<div class="second-panel-post">
  ...
  <p class="first-panel-post-title episode-number"><?= mb_strimwidth($episode->getTitle(), 0, 2) ?></p>
  <p class="first-panel-post-title"><?= mb_strimwidth($episode->getTitle(), 2, 100) ?></p>
  <p class="first-panel-post-date"><?= $episode->getCreationDate() ?></p>
  <div class="first-panel-post-content"><?= $episode->getContent() ?></div>
</div>
...
<div class="second-level-first-panel-post">
  ...
  foreach ($episodeComments as $key => $value) {
    ...
    if ($value->getReported() === '1' and isset($_GET['ticket'])) {
      ...
      <a class="report-it" href="index.php?action=report&amp;comment=<?= $thisComment ?>&amp;id=<?= htmlspecialchars($_GET['episode']) ?>&amp;ticket=<?= htmlspecialchars($_GET['ticket']) ?>&amp;episode=<?= htmlspecialchars($_GET['episode']) ?>">signaler un abus<br /></a>
      <?php
      ...
      <?php
        ...
        $thisComment = $value->getCommentId(); ?>
      <a class="report-it" href="index.php?action=report&amp;comment=<?= $thisComment ?>&amp;id=<?= $lastEpisode->getPostId() ?>&amp;ticket=<?= $lastTicket->getPostId() ?>&amp;episode=<?= $lastEpisode->getPostId() ?>">signaler un abus<br /></a>
      <?php
    } ?>
    <br />
    <?php
...
if (isset($_GET['episode']) and $episodeCheck->getType() === 'episode' and $episodeCheck->getPostId() != null) {
    ?>
  <form class="episode-comment-form"  action="index.php?action=addComment&amp;ticket=<?= htmlspecialchars($_GET['ticket']) ?>&amp;episode=<?= htmlspecialchars($_GET['episode']) ?>&amp;type=episode&amp;post=<?= htmlspecialchars($_GET['episode']) ?>" method="post" id="episode-comment-form">
    <div>
